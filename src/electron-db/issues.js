...
function allFieldsNull(issueData) {
  for (const val of Object.values(issueData)) {
...
exports.getAllIssues = function (projectId) {
  ...
  return exports.queryIssues(projectId, []);
};

exports.createIssue = function (projectId, issueData) {
  try {
    if (allFieldsNull(issueData)) {
      console.warn(
        "[electron-db] createIssue called with all-null fields, skipping creation",
      );
      return null;
    }
    ...
    const insert = db.prepare(
      "INSERT INTO issues (id, project_id, data, created_at, updated_at) VALUES (?,?, ?, ?, ?)",
    );
    const info = insert.run(id, projectId, JSON.stringify(dataToStore), created, created);
    if (info.changes) {
...
exports.updateIssue = function (issueId, issueData) {
  try {
    if (allFieldsNull(issueData)) {
      console.warn(
        "[electron-db] updateIssue called with all-null fields, skipping update",
      );
      return null;
    }
    ...
    const update = db.prepare(
      "UPDATE issues SET data = ?, updated_at = ? WHERE id = ?",
    );
    ...
    if (info.changes) {
      // re-index dynamic fields for filtering
      try {
        exports.indexIssue(issueId, dataToStore, updated);
      } catch (err) {
        console.error("[electron-db] indexIssue error", err);
      }
...
exports.indexIssue = function (issueId, issueData, timestamp) {
  ...
  const insert = db.prepare(
    "INSERT INTO issue_index (issue_id, field_name, value_text, value_number, value_date) VALUES (@issue_id, @field_name, @value_text, @value_number, @value_date)",
  );
...
exports.queryIssues = function (projectId, filters) {
  if (!filters || filters.length === 0) {
    //const stmt = db.prepare("SELECT * FROM issues ORDER BY created_at DESC");
    const stmt = db.prepare("SELECT * FROM issues WHERE project_id = ? ORDER BY created_at DESC");
  ...
  filters.forEach((f, idx) => {
    const alias = `idx${idx}`;
    joins.push(
      `JOIN issue_index ${alias} ON ${alias}.issue_id = issues.id AND ${alias}.field_name = ?`,
    );
    params.push(f.field);
    ...
    if (f.op === "eq") {
      ...
      const dateParam = isDateLike(f.value)
  ...
  const sql = `SELECT DISTINCT issues.* FROM issues ${joins.join(" ")} WHERE issues.project_id = ? ORDER BY issues.created_at DESC`;
