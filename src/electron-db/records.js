...
exports.getRecords = function (projectId) {
  try {
    ...
    const stmt = db.prepare('SELECT * FROM records WHERE project_id = ? ORDER BY created_at DESC')
...
exports.createRecord = function (projectId, title) {
  try {
    ...
    const ins = db.prepare('INSERT INTO records (project_id, title, comments, created_at, updated_at) VALUES (?, ?, ?, ?, ?)')
...
exports.updateRecord = function (recordId, patch) {
  try {
    ...
    const nextComments = Array.isArray(patch.comments) ? patch.comments : (() => { try { const a = JSON.parse(existing.comments); return Array.isArray(a) ? a : [] } catch { return [] } })()
    const upd = db.prepare('UPDATE records SET title = ?, comments = ?, updated_at = ? WHERE id = ?')
    const info = upd.run(nextTitle, JSON.stringify(nextComments), nowISO(), recordId)
    if (info.changes) {
...
exports.deleteRecord = function (projectId, recordId) {
  try {
    if (projectId == null) throw new Error('Invalid projectId')
    const del = db.prepare('DELETE FROM records WHERE id = ? AND project_id = ?')
